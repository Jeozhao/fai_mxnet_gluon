...
def progressbar(i, n, bar_len=40):
    ...
    print('[%s] %s%s' % (prog_bar, percents, '%'), '\r')
...
def train():
    ...
    for epoch in range(epochs):
        ...
        logging.info('task: %s  [Epoch %d] Train-acc: %.3f, mAP: %.3f, loss: %.3f | Val-acc: %.3f, mAP: %.3f, loss: %.3f | time: %.1f' %
                 (task, epoch, train_acc, train_map, train_loss, val_acc, val_map, val_loss, time.time() - tic))
